...
// @Harzi-Edition: WWeiterentwicklung ab Version 1.1.0
// @contributor    Harzi66
// @contributor    Spezial Thanks for testing @zillen4K & @nExt_mAnDo
// @match          https://*.alliances.commandandconquer.com/*/index.aspx*
// @updateURL      https://raw.githubusercontent.com/Harzi66/CnC-TA-Lister-UI-Harzi-Edition/main/CnC-TA_Lister_UI.meta.js
// @downloadURL    https://raw.githubusercontent.com/Harzi66/CnC-TA-Lister-UI-Harzi-Edition/main/CnC-TA_Lister_UI.user.js
...
// 1.25.0-Beta1
// - Neu
// - Einführung Konfiguration die das Startverhalten der angezeigten Spalten beeinflusst.
// - Basenname ist jetzt anklickbar (identisches Verhalten wie die Koordinaten)
// - Renderer-System überarbeitet und vereinheitlicht.
// - Spaltenbreiten und Reihenfolge angepasst
// - Standard Ansicht nach Start festgelegt
// - Unterstützung der Spaltenausrichtung (align) für alle Renderer.
// - Interne Codeverbesserungen und höhere Robustheit.
/*
==========================================================
Harzi Edition Development Roadmap
==========================================================
    ...
    const ListerUIScript = async () => {
        ...
        window.harzi.core.helpers.deepMerge = function(defaults, saved) {
            ...
            for (const key in defaults) {
                ...
                if (
                    ...
                    result[key] = window.harzi.core.helpers.deepMerge(
                        defaults[key],
                        saved[key]
                    );
        ...
        window.harzi.core.settings = {
            ...
            load() {
                ...
                if (saved) {
                    this.data = window.harzi.core.helpers.merge(
                        this.defaults,
                        JSON.parse(saved)
                    );
                }
        ...
        function tabAllianceCities() {
            ...
            columnNames.sort((a, b) => {
                ...
                const orderA = colA?.defaultOrder ?? Number.MAX_SAFE_INTEGER;
                const orderB = colB?.defaultOrder ?? Number.MAX_SAFE_INTEGER;
            ...
            const savedColumns = savedSettings?.allianceCities || {};
            ...
            columnNames.forEach((columnName, index) => {
                ...
                if (definition) {
                    ...
                    if (savedColumns.hasOwnProperty(definition.id)) {
                        visible = savedColumns[definition.id];
                    } else {
                ...
                tableColumnModel.setColumnVisible(index, visible);
        ...
        function tabSettings() {
            ...
            function showColumnConfiguration(pageContainer, title, columns) {
                ...
                columns.forEach(column => {
                    ...
                    checkBox.setValue(column.default ?? false);

                    if (column.default === undefined) {
                        console.error("default fehlt:", column.id, column);
                    }
                    ...
                    checkBox.addListener("changeValue", function () {
                        ...
                        const columnIndex = window.columnIndexMap[column.field];

                        if (columnIndex >= 0) {

                            window.allianceCitiesTable
                                .getTableColumnModel()
                                .setColumnVisible(columnIndex, checkBox.getValue());
            ...
            function showProfilePage(pageContainer) {
                ...
                pageContainer.add(
                    new qx.ui.basic.Label("Hier werden später die Profileinstellungen angezeigt.")
                );
        ...
        function getSettingsData() {
            ...
            window.columnDefinitions.allianceCities.forEach(column => {
                ...
                settings.allianceCities[column.id] =
                    column.checkBox ? column.checkBox.getValue() : column.default;
        ...
        function loadSettings() {
            ...
            try {
                ...
                console.warn(
                    "Harzi Edition: Gespeicherte Einstellungen konnten nicht gelesen werden."
                );
        ...
        function applySettings(settings) {
            ...
            if (settings.allianceCities) {

                window.columnDefinitions.allianceCities.forEach(column => {

                    if (
                        column.checkBox &&
                        settings.allianceCities.hasOwnProperty(column.id)
                    ) {
                        column.checkBox.setValue(
                            settings.allianceCities[column.id]
                        );
                    }
